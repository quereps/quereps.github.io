...
const reportTemplates = {
  placeData: {
      ...
      availabilityList:{
        ...
        title:"SKU List",
        logo:"https://app.form.com/fs/v1/h/aMdkGmN080RYiT1L7rDWkrKEKuFd-b76KN8Wt-djYao/277611.png",
        columns:["upc","name","availabilityStatus","availabilityStatus"],
        options:{
            legend:false,
          exclude:["Undefined"],
...
function JSONToGraph(jsonArray, title, type, destination, settings) {
    ...
    canvas.style.width = "calc(100% - 100px)"; //canvas.width = 250;
    canvas.style.height = "calc(100% - 20px)"; //canvas.height = 250;
...
function createHTMLSection(id,name, imageURL,type, settings){
        ...
        if (title) {
          headerHTML = '<div class="sectionHeader"><h3>';
          headerHTML += (imageURL ? '<img height="40" src="'+imageURL+'"/>' : '');
          headerHTML += title ? title : '';
          headerHTML += '</h3><div class="score"></div></div>';
        }
  ...
  //console.log("skuList: ",skuList);
    ...
    for(var element in report){
      ...
      if(current.type=="skuList"){
        JSONToHTMLTable(rankObjects(skuList, current.options.rankBy || "facings", current.columns), containerId, current.options) 
      }
